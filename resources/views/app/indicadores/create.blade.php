...
@section('title', 'Cadastro de Indicador')
    

@section('content_header')
    <h1 class="m-0 text-dark">Cadastro de Indicador</h1>
@stop
                        ...
                        <form method="post" action="{{route('indicador.store')}}" enctype="multipart/form-data" onsubmit="bloqSubmit()">
                            ...
                            <div class="input-group mb-3">
                                ...
                                <select class="custom-select rounded-0" id="id_servico" name="id_user_tec">
                                    @foreach ( $tecnicos as $tecnico )
                                        <option value='{{$tecnico->id}}'{{ old('id_user_tec') === $tecnico->id ? 'selected' :''}}>{{$tecnico->name}}</option>    
                                    @endforeach
                                </select>
                            ...
                            <div class="input-group mb-3">
                                ...
                                <select class="custom-select rounded-0" id="id_servico_tipo" name="id_servico_tipo">
                                    @foreach ( $servicos as $servico )
                                        <option value='{{$servico->id}}'{{ old('id_servico_tipo') === $servico->id ? 'selected' :''}}>{{$servico->nome}}</option>    
                                    @endforeach
                                </select>
                                <div class="input-group-append">
                                    <span class="input-group-text"><i class="fas fa-solid fa-asterisk"></i></span>
                            ...
                            <div class="input-group mb-3">
                                ...
                                <select class="custom-select rounded-0" id="mes" name="mes">
                                    @for($i = 1; $i < 13; $i++)
                                        @if ($i < 10 )
                                            <option value='0{{$i}}'{{ '0'.$i === DATE('m') ? 'selected' :''}}>0{{$i}}</option>                                            
                                        @else
                            ...
                            <div class="input-group mb-3">
                                ...
                                <input id="qtd" type="text" class="form-control" placeholder="Quantidade" name="qtd" value="{{old('qtd')}}">
                                <div class="input-group-append">
                                    <span class="input-group-text"><i class="fas fa-solid fa-asterisk"></i></span>
                                </div>
                            </div>
